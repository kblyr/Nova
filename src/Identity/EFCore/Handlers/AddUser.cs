...
sealed class AddUserHandler : IRequestHandler<AddUserCommand>
{
    ...
    public async Task<IResponse> Handle(AddUserCommand request, CancellationToken cancellationToken)
    {
        ...
        await _mediator.Publish(user.Adapt<User, UserAddedEvent>(), cancellationToken);
        await _mediator.Publish(user.Adapt<User, SendUserCreationConfirmationMailRequestedEvent>() with 
        {
